...
def self_train_net(net, net_pseudo, cfg, gpu):
    ...
    with torch.no_grad():
        ...
        pre_dir = '/content/drive/MyDrive/Project/IAST2/t4_log/gta5/final6/sl_2/pseudo/label'
        ...
        if cfg.DATASET.TARGET.PSEUDO_PL == "IAST":
            ...
            for i, b in enumerate(t_train_data_sl):
                ...
                for _i, name in enumerate(names):
                    ...
                    pre_path = os.path.join(pre_dir , pseudo_label_name)
                    #pre_label = Image.open(pre_path).resize((1536,768))
                    pre_label = Image.open(pre_path)
                    pre_label = np.asarray(pre_label)

                    tmp = np.copy(label)
                    mask1 = np.zeros(pre_label.shape, dtype=np.uint8)
                    mask2 = np.ones(label.shape, dtype=np.uint8)
                    mask1[tmp==255] = 1
                    mask2[tmp==255] = 0

                    label = np.multiply(pre_label, mask1) + np.multiply(tmp, mask2)
    ...
    print(cfg.TRAIN.EPOCHES * len(train_data))
